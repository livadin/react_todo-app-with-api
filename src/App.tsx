...
export const App: React.FC = () => {
  ...
  const handleUpdateTodo = async (
    ...
    try {
      ...
      setErrorMessage(ERROR_TEXT.update);
      throw e;
    } finally {
      setUpdatingIds(prev => {
        const copy = new Set(prev);
  ...
  const handleToggleAll = async () => {
    ...
    const updates = todosToUpdate.map(async todo => {
      try {
        await updateTodo(todo.id, { completed: newCompleted });

        return { id: todo.id, ok: true };
      } catch {
        return { id: todo.id, ok: false };
      }
    });
    ...
    setTodos(prev =>
      prev.map(todo =>
        ids.includes(todo.id) ? { ...todo, completed: newCompleted } : todo,
      ),
    );
